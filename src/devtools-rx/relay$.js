...
export default connection$.mergeMap(connection => {
  return Observable.merge(
    connection.disconnect$.withLatestFrom(connections$).do(([_, connections]) => {
      if (opened[connection.id]) {
        let count = 0
        for (const id of Object.keys(opened[connection.id])) {
          if (connections[id]) {
            connections[id].postMessage({ command: 'DISCONNECTED', from: connection.id, name: connection.name })
            count++
          }
        }
        delete opened[connection.id]
        debug && console.log('client[' + connection.name + ' ' + connection.id + '] disconnected from ' + count + ' clients')
      } else {
        debug && console.log('client[' + connection.name + ' ' + connection.id + '] disconnected')
      }
    }),
    connection.message$.withLatestFrom(connections$).do(([message, connections]) => {
      if (message.broadcast) {
        ...
        if (!target) {
          debug && console.log('Couldn\'t relay message', message, connections)
        ...
        delete _message.to
        debug && console.log('Relay message from ' + connection.id + ' to ' + target.id, _message)
